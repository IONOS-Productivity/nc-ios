...
class NCMenuAction {
    ...
    init(title: String, boldTitle: Bool = false, destructive: Bool = false, details: String? = nil, icon: UIImage, isHeader: Bool = false, order: Int = 0, accessibilityIdentifier: String? = nil, sender: Any?, action: ((_ menuAction: NCMenuAction) -> Void)?) {
...
extension NCMenuAction {
    ...
    static func selectAllAction(sender: Any?, action: @escaping () -> Void) -> NCMenuAction {
        NCMenuAction(
			...
			sender: sender, action: { _ in action() }
        )
    }
