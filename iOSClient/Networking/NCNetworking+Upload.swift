...
extension NCNetworking {
    func upload(metadata: tableMetadata,
        ...
        if database.getTableAccount(account: metadata.account)?.autoUploadOnlyNewSinceDate == nil {
            self.database.updateAccountProperty(\.autoUploadOnlyNewSinceDate, value: metadataCreationDate, account: metadata.account)
        } else if metadata.sessionSelector == NCGlobal.shared.selectorUploadAutoUpload,
           let autoUploadOnlyNewSinceDate = database.getTableAccount(account: metadata.account)?.autoUploadOnlyNewSinceDate {

            if autoUploadOnlyNewSinceDate < metadataCreationDate {
                self.database.updateAccountProperty(\.autoUploadOnlyNewSinceDate, value: metadataCreationDate, account: metadata.account)
            }
        }
